...
class QuickVariantSelects extends HTMLElement {
  ...
  updatePrice(variant) {
    ...
    if (variant && price) price.innerHTML = `<span>${Shopify.formatMoney(variant.price, Shopify.money_format)}</span>
...
class QuickProductForm extends HTMLElement {
  ...
  onAddCartClicked() {
    ...
    const freeGiftData = JSON.parse(document.getElementById('FreeGiftData').textContent) || [];
